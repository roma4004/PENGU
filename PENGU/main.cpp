...
void CreateWorld() {
	...
	srand(time(0));
	OfMountains[0][0] = -10;// полюбас гора с точки -10 слева
	OfMountains[0][1] = (rand() % 12) + 10; //рандом высоты
	OfMountains[1][0] = WIDTH_MAP - 10; // полюбас гора с точки справа
	OfMountains[1][1]= (rand() % 12) + 10; //рандом высоты
	//рандомим параметры горы координата и высота	
	 // нужная штуковина
	for (int i = 2; i < 30; i++) {// для 30 гор под завязку массива
